...
@section('content')
  ...
  <div class="full-section alt">
    <section>
      ...
      <p class="section-sub">Four interconnected systems that transform studying into an immersive neo-Tokyo experience.
      </p>
      <div class="features-grid">
        <a href="{{ route('study.index') }}" class="feature-card">
          <span class="fc-icon">◈</span>
          <div class="fc-title">Virtual Study Tables</div>
          <div class="fc-desc">Join global focus rooms with Pomodoro timers, shared notes, and ambient table lighting that
            shifts with your session state.</div>
          <div class="fc-tag c">Live Now</div>
        </a>
        <a href="{{ url('/barista') }}" class="feature-card purple">
          <span class="fc-icon">◆</span>
          <div class="fc-title">AI Barista</div>
          <div class="fc-desc">NEXUS-7 remembers your orders, recommends music for your mood, drops cyberpunk wisdom, and
        ...
        <a href="{{ url('/cafe') }}" class="feature-card pink">
          ...
          <div class="fc-desc">Holographic messages drift across the café. Global lounge, table chat, private DMs — all
            with neon visual effects.</div>
          <div class="fc-tag pk">Real-time</div>
        </a>
        <a href="{{ url('/music') }}" class="feature-card">
          <span class="fc-icon">▷</span>
          <div class="fc-title">Music Lounge</div>
          <div class="fc-desc">Lo-fi, synthwave, rain ambience, cyberpunk drive — reactive audio waves that pulse with the
            beat of the city.</div>
          <div class="fc-tag c">5 Channels</div>
        </a>
        <a href="{{ url('/profile') }}" class="feature-card purple">
          <span class="fc-icon">⬡</span>
          <div class="fc-title">Digital Economy</div>
          <div class="fc-desc">Earn credits by studying. Unlock café themes, holographic avatars, cyber pets, and
            exclusive room effects.</div>
          <div class="fc-tag p">Credits System</div>
        </a>
        <a href="{{ url('/cafe') }}" class="feature-card pink">
          <span class="fc-icon">⬟</span>
          <div class="fc-title">Dynamic Weather</div>
          <div class="fc-desc">The café atmosphere mirrors real-world weather. Rainy nights intensify neon reflections and
            storm ambience.</div>
  ...
  <div class="full-section">
    <section>
      ...
      <p class="section-sub">Join an active table or create your own focus space. Table lights reflect the energy inside.
      </p>
      <div class="tables-preview">
        @foreach($globalTables as $table)
            @php
                $colorHex = '#1a6aff';
                if ($table->color === 'purple') $colorHex = 'var(--purple)';
                if ($table->color === 'red') $colorHex = 'var(--pink)';
                if ($table->color === 'green') $colorHex = '#00ff88';

                $cssClass = 'blue';
                if ($table->color === 'purple') $cssClass = 'violet';
                if ($table->color === 'red') $cssClass = 'red';
                if ($table->color === 'green') $cssClass = 'green';
                
                $usersCount = \App\Models\StudySession::where('study_table_id', $table->id)->where('active', true)->count();
            @endphp
            <div class="table-card {{ $cssClass }}" onclick="location.href='{{ route('study.show', $table->id) }}'" style="cursor:pointer">
                <div class="tc-glow" style="background:{{ $colorHex }}"></div>
                <div class="tc-status">
                    <div class="tc-dot" style="background:{{ $colorHex }};box-shadow:0 0 6px {{ $colorHex }}"></div>
                    <span style="color:{{ $colorHex }}">{{ strtoupper($table->activity) }}</span>
                </div>
                <div class="tc-name">{{ strtoupper($table->name) }}</div>
                <div class="tc-users">{{ $usersCount }} users connected</div>
                <div class="tc-bar">
                    <div class="tc-bar-fill" style="width:{{ min(100, max(5, $usersCount * 10)) }}%;background:{{ $colorHex }}"></div>
                </div>
            </div>
        @endforeach
        <div class="table-card blue" style="border-style:dashed;opacity:0.5;cursor:pointer;"
          onclick="location.href='{{ route('study.index') }}'">
          <div
  ...
  <div class="full-section">
    <section>
      ...
      <p class="section-sub">Your AI barista, motivation engine, and cyberpunk companion. Powered by neural networks,
        fueled by espresso.</p>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:60px;align-items:center;">
        <div>
          <div style="margin-bottom:24px">
            <div
              style="font-family:var(--font-mono);font-size:0.62rem;letter-spacing:0.15em;color:var(--text-dim);margin-bottom:8px">
              CAPABILITIES</div>
            <ul style="list-style:none;display:flex;flex-direction:column;gap:10px">
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--cyan);font-size:0.8rem">►</span> Music recommendations by mood</li>
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--purple);font-size:0.8rem">►</span> Personalized motivation phrases</li>
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--pink);font-size:0.8rem">►</span> Cyberpunk quote generator</li>
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--cyan);font-size:0.8rem">►</span> Coffee suggestions by mood</li>
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--purple);font-size:0.8rem">►</span> Study session planning</li>
              <li style="display:flex;align-items:center;gap:10px;font-size:0.88rem"><span
                  style="color:var(--pink);font-size:0.8rem">►</span> Remembers your preferences</li>
            </ul>
          </div>
        ...
        <div class="chat-preview">
          ...
          <div class="chat-messages" id="demo-chat">
            <div class="msg ai">
              <div class="msg-bubble">Welcome to Cyber Café 2099. I'm NEXUS-7. What can I brew for your mind tonight?
              </div>
            </div>
            <div class="msg user">
            ...
            <div class="msg ai">
              <div class="msg-bubble">Initiating neural sync... I'd suggest <span style="color:var(--cyan)">Synthwave
                  Drive</span> — 80 BPM, sub-bass resonance, perfect for flow state. Shall I queue it?</div>
            </div>
          </div>
